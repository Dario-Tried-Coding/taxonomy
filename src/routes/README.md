...
- [ ] `/[locale]/page.info.ts`: Add search typing to if the page supports search paramaters
- [ ] Convert `Link` components for `/[locale]` to `<Locale.Link>`
- [ ] Convert `params` typing in `/[locale]/page.ts` to `z.infer<>`
